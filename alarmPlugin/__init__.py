...
class alarmPlugin(Plugin):
    localizations = {
        'Alarm': {
            "settingAlarm": {
                "en-US": u"Wecker stellen\u2026",
                "fr-FR": u"Réglage de l'alarme\u2026"
            }, "alarmWasSet": {
                "en-US": "Dein Wecker ist gestellt auf {0}:{1} {2}.",
                "fr-FR": u"Votre alarme est programmée pour {0}:{1} {2}"
            }, "alarmSetWithLabel": {
                "en-US": "Dein Wecker {0} {1} ist gestellt fuer {2}:{3} {4}.",
    ...
    res = {
        'setAlarm': {
            'de-DE': '.*stell.* wecker auf.* (0?[1-9]|1[012])([0-5]\d)?\s?([APap][mM])\s?(\bcalled|named|labeled\b)?\s?(([a-z0-9]{1,7}\s)?([a-z0-9]{1,7})\s?([a-z0-9]{1,7}))?',
